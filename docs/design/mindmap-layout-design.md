...
| LayoutService | 有状态的布局服务，管理尺寸缓存、订阅 Action 变化、驱动 Engine 计算 | MindmapLayoutServiceImpl      |
...
| Action 类型          | 处理逻辑                             |
| -------------------- | ------------------------------------ |
| `addChildNode`       | 测量新节点尺寸 → 更新布局            |
| `updateNode`         | 清除旧尺寸缓存 → 重新测量 → 更新布局 |
| `deleteNode`         | 直接更新布局                         |
| `toggleCollapseNode` | 直接更新布局                         |
...
### 2. 为什么使用 Action 订阅而非 Store 订阅？

- Action 携带变化的语义信息（如新增的节点对象）
- 可以精确响应特定操作，避免不必要的重计算
- 符合 Action Layer 的设计理念
