...
├── __mocks__/                  # Jest 模拟文件
├── app/                        # Next.js 14 App Router 应用目录
...
- 定义应用的路由结构
- 包含页面组件和布局
- 使用 Next.js 14 App Router 的文件系统路由
...
- 本项目使用 **Server Actions** (`lib/actions/`) 而非传统的 API Routes
- 这是 Next.js 14 推荐的数据变更方式，提供更好的类型安全和性能
...
- **`domain/`**: 核心架构层，实现 Command/Action 模式
  ...
  - 支持完整的撤销/重做功能
  - 命令验证和批量操作
- **`actions/`**: Server Actions（Next.js 14 服务端数据操作）
...
- 工具函数使用 camelCase 命名
- Server Actions 放在 `lib/actions/` 目录
...
```typescript
// lib/actions/mindmap-actions.ts
"use server";
...
| 2025-11-06 | 2.0.0 | 添加 lib/domain/ 架构说明，更新 Server Actions 说明，补充命令系统文档 | Claude |
